...
def _module_name(class_name: str) -> str:
    """Snake-case module name for a page-object class: HomePageFormPage → home_page_form_page."""
    s = re.sub(r"(?<=[a-z0-9])(?=[A-Z])", "_", class_name).lower()
    s = re.sub(r"[^0-9a-z]+", "_", s).strip("_") or "page"
    return "m_" + s if s[0].isdigit() else s


def _dq(s: str) -> str:
    """A safe double-quoted Python string literal for generated source."""
...
def inventory_to_context(inv: CoverageInventory) -> dict[str, Any]:
    ...
    # One module per page object (the pro POM layout): module names derive from the class
    # name and are deduped so two pages that normalise identically can't clobber each other.
    seen_modules: set[str] = set()
    for p in pages:
        module, n = _module_name(p["class_name"]), 2
        while module in seen_modules or module in ("base_page", "__init__"):
            module = f"{_module_name(p['class_name'])}_{n}"
            n += 1
...
def _render_page_module(page: dict[str, Any], project_name: str) -> str:
    """One page-object module (the pro POM layout: a file per page, inheriting BasePage).

    Rendered in Python rather than Copier because Copier can't emit N files from a list;
    the same post-copy pattern as login.py / schemas.json. Pure and deterministic."""
    cls = page["class_name"]
    desc = " ".join(str(page["description"] or page["name"]).split())
    lines = [
        f'"""{cls} — page object for {page["location"]!r} on {project_name}.',
        "",
        "Generated by Aitomation Scaffold from the discovered page. Locators are seeded from",
        "observed fields; confirm them and extend the class as the page evolves.",
        '"""',
        "",
        ...
        f'    """{desc}"""' if desc else '    """(no description discovered)"""',
        "",
        f"    PATH = {_dq(page['location'])}",
    ]
    if page["fields"]:
        lines += [
            "",
        ...
        lines += [f"        self.{f['attr']} = page.{f['locator']}" for f in page["fields"]]
    if page["is_form"] and page["fields"]:
        first = page["fields"][0]["attr"]
        lines += [
            "",
            f'    def fill(self, **values: str) -> "{cls}":',
            f'        """Fill discovered fields by attribute name, e.g. .fill({first}="...")."""',
...
def _write_page_modules(dest: Path, context: dict[str, Any]) -> None:
    """Emit pages/<module>.py per discovered page, and drop any page module a PREVIOUS
    scaffold generated for a page that no longer exists (recognised by our header marker —
    a hand-written module is never touched)."""
    pages_dir = dest / "pages"
    if not context["has_browser"] or not pages_dir.is_dir():
        return
    written: set[str] = set()
    for page in context["pages"]:
        name = f"{page['module']}.py"
        (pages_dir / name).write_text(
            _render_page_module(page, context["project_name"]), encoding="utf-8"
        )
        written.add(name)
    keep = written | {"__init__.py", "base_page.py"}
    for p in pages_dir.glob("*.py"):
        if p.name in keep:
            continue
        try:
            if "Generated by Aitomation Scaffold" in p.read_text(encoding="utf-8"):
                p.unlink()
        except OSError:
            continue


# Pre-package-layout files this tool used to generate at the scaffold root, with the marker
# proving WE wrote them. A re-scaffold upgrades the run in place; a hand-written file whose
# name merely collides is never deleted.
_LEGACY_FILES = {
    "pages.py": "Generated by Aitomation Scaffold",
    "api_client.py": "Generated by Aitomation Scaffold",
    "reporting.py": "Reporting hook — intentionally left as an interface",
}


def _remove_legacy_files(dest: Path) -> None:
    for name, marker in _LEGACY_FILES.items():
        p = dest / name
        try:
...
def scaffold_project(
    ...
    # Page-object modules are written post-copy, one file per discovered page (Copier can't
    # emit N files from a list); pages/__init__.py re-exports them so `from pages import X`
    # is the stable import contract the Write stage relies on.
    _write_page_modules(dest, context)
    _remove_legacy_files(dest)
