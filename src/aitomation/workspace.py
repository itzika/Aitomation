...
Shared infrastructure for BOTH front-ends — the CLI and the TUI resolve a system's scaffold
and drafted tests through this one layout, so artifacts produced by either are visible to the
other (the TUI library lists them; `aitomation enable` finds them). Layout is visible and
organised by the tested app, with one timestamped directory per generation run so nothing
clobbers and runs are diffable/archivable:
...
class Workspace:
    ...
    def ensure_run(self, slug: str) -> Path:
        """The run directory to scaffold/draft into: reuse the latest run if there is one,
        else create a fresh one. The single policy both front-ends share so re-scaffolding
        refreshes a run in place (keeping prior drafts) instead of orphaning it."""
        return self.latest_run(slug) or self.new_run(slug)
    ...
    def save(
        ...
        *,
        ...
        unchanged re-discover never makes you scaffold/write from scratch again. An omitted
        `origin` likewise preserves the prior one (falling back to the inventory's base_url for
        a brand-new system) — so a CLI scaffold/write can upsert the index without clobbering
