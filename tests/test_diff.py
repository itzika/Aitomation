...
def _inv(elements, journeys=()):
    return CoverageInventory(
        ...
        suggested_journeys=list(journeys),
    )
...
def test_element_key_ignores_llm_name():
    ...
    page_b = Element(
        kind="page", name="Landing Page", location="/", description="d", priority="high"
    )
...
def test_diff_changed_by_inputs_but_not_by_advisory_fields():
    ...
    new_advisory = _inv(
        [_ep("create", "/things", method="POST", desc="new words!", priority="high", inputs=title)]
    )
...
def test_fingerprint_stable_across_input_order():
    a = _ep(
        "x", "/x", inputs=[InputField(name="a", where="query"), InputField(name="b", where="query")]
    )
    b = _ep(
        "x", "/x", inputs=[InputField(name="b", where="query"), InputField(name="a", where="query")]
    )
...
def test_diff_no_new_flows_when_only_journey_names_change():
    ...
    old = _inv(
        els,
        [Journey(name="Browse things", description="d", priority="high", elements=["list", "get"])],
    )
    new = _inv(
        els,
        [
            Journey(
                name="Thing Discovery Path",
                description="d",
                priority="high",
                elements=["list", "get"],
            )
...
def test_diff_no_new_flows_when_journeys_regrouped():
    ...
    old = _inv(
        els,
        [
            Journey(name="AB flow", description="d", priority="high", elements=["a", "b"]),
    ...
    new = _inv(
        els,
        [
            Journey(name="A solo Path", description="d", priority="high", elements=["a"]),
...
def test_diff_new_flow_only_when_surface_actually_grows():
    old = _inv(
        [_ep("a", "/a")], [Journey(name="A", description="d", priority="high", elements=["a"])]
    )
